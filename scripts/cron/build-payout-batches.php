...
try {
    ...
    foreach ($byPartner as $partnerId => $data) {
        ...
        try {
            ...
            // Attach conversions to the batch.
            $placeholders = implode(',', array_fill(0, count($data['ids']), '?'));
            $params = array_merge([$batchId], $data['ids']);
            Database::query(
                "UPDATE conversions SET payout_batch_id = ?, updated_at = NOW()
                 WHERE id IN ($placeholders) AND status = 'payable' AND payout_batch_id IS NULL",
                $params
            );
